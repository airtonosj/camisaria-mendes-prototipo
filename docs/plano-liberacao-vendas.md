...
| --- | --- | --- | --- |
| 1. Artes permanentes | `UPLOADS_DIR` obrigatório fora da release, health check, backup dos uploads e prova antes/depois do redeploy | Prova local criada e lida; 4 artes copiadas e restauradas | Configurar o caminho persistente real da Hostinger e repetir `create` antes / `check` depois de um redeploy |
...
| 3. Produção | Validação de HTTPS, proxy, SMTP, banco restrito e storage; checkout fechado não impede mais o boot | Configuração insegura é recusada automaticamente | Definir `APP_ENV=production` no hPanel com os valores reais e obter health verde |
...
| 7. Políticas | Páginas públicas de privacidade, trocas/reembolso e atendimento, ligadas no rodapé e no checkout | TypeScript e build validam as rotas | Confirmar razão social/documento/endereço comercial, horário de atendimento e revisão jurídica final |
| 8. Checkout público | Trava única em variável de ambiente | Continua `false` no ambiente local | Alterar para `true` somente após todas as linhas acima ficarem verdes no servidor publicado |
