...
pedidos permanece bloqueado até o checkout InfinitePay estar integrado.
Ele assume uma **VPS da Hostinger** com acesso SSH, porque a hospedagem compartilhada não
mantém um processo Node. O checkout automático e o webhook continuam reservados para a
última fase; hoje o pagamento é Pix conferido por uma pessoa, e é isso que a tela diz.
...
GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, ALTER, DROP, INDEX, REFERENCES,
      CREATE VIEW, SHOW VIEW ON camisaria_mendes.* TO 'camisaria_migrator'@'localhost';
...
A API avisa no boot quando o token é curto, quando a chave Pix está vazia, quando o
...
HTTPS, Pix, SMTP, usuário restrito do banco, diretório persistente de uploads ou se o modo
de pagamento não for `manual`. A chave estática de manutenção fica desabilitada por padrão;
...
4. um pedido de teste aparece no painel como pendente, com a chave Pix real na tela do
   aluno e o botão de WhatsApp apontando para o representante da campanha;
...
## 10. O que já está preparado para a InfinitePay

A integração é a Fase 5, mas o terreno está pronto:

- `PAYMENT_PROVIDER=infinitepay` e `INFINITEPAY_HANDLE` já são lidos em `api/config.mjs`;
- `GET /api/settings` informa os meios aceitos e mantém o checkout desabilitado até a
  integração estar validada;
...
Faltam a criação do link em `POST /links`, o redirecionamento, o endpoint público de webhook
e a conferência por `POST /payment_check`. Só depois dos testes de idempotência e valor
correto `INFINITEPAY_CHECKOUT_ENABLED` pode mudar para `true`.
