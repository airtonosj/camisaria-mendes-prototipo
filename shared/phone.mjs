...
export function parseWhatsapp(value) {
  if (typeof value !== 'string' || /[^0-9+()\s.-]/.test(value)) {
    return { error: 'Informe somente o telefone com DDD, sem letras ou outros caracteres.' };
  }
  if ((value.match(/\+/g) ?? []).length > 1 || value.includes('+') && !value.trim().startsWith('+55')) {
    return { error: 'Use o código do Brasil (+55) antes do DDD.' };
  }
